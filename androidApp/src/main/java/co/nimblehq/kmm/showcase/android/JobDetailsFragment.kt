        ...
        with(binding) {
            with(jobDetailUiModel) {
                listOf(
                    ...
                    it.text = getString(R.string.job_details_price_by_area_value, minimumPrice, maximumPrice)
                }

                lJobDetailsDateTime.run {
                    tvJobDetailsDate.text = confirmedAt.formatDate()
                    tvJobDetailsTime.text =
                        getString(R.string.job_details_date_time_time_value, timeSlotStart, timeSlotEnd)
                }
